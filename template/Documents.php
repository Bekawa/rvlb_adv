...
<div>
    <input type="text" id="geojsonInput" placeholder="Enter GeoJSON data">
    <button onclick="drawLine()">Preview</button>
  </div>
  <div id="map"></div>
  ...
  <script>
    ...
    function drawLine() {
      var geojsonDataInput = document.getElementById("geojsonInput").value;
      var geojsonData;

      try {
        geojsonData = JSON.parse(geojsonDataInput);
      } catch (error) {
      ...
      lineLayer = L.geoJSON(geojsonData).addTo(map);
      map.fitBounds(lineLayer.getBounds());
    }

    function initializeMap() {
      map = L.map('map').setView([7.651700336048904, 38.28986544000034], 10);
    ...
    initializeMap(); // Call the initializeMap() function to set up the map when the page loads.
  </script>
